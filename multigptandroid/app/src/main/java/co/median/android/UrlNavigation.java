...
public class UrlNavigation {
    ...
    private boolean isInternalUri(Uri uri) {
        ...
        // Keep Alipay gateway inside the main WebView.
        // Otherwise, the shell may open a CustomTab/external browser for the HTTPS gateway
        // and then Alipay will also open the native app, resulting in a double pop-up.
        String host = uri.getHost();
        if (host != null) {
            String h = host.toLowerCase(Locale.ROOT);
            if (h.endsWith("alipay.com") || h.endsWith("alipaydev.com")) {
                return true;
            }
        }
...
//		Log.d(TAG, "shouldOverrideUrl: " + url);
        ...
        if ("wechat-login".equals(uri.getScheme())) {
            if (noAction) return true;

            String callback = uri.getQueryParameter("callback");
            mainActivity.startWeChatLogin(callback);
            return true;
        }
